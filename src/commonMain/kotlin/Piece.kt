    ...
    fun moveChecker(oldPos: Pair<Int, Int>, newPos: Pair<Int, Int>, withCheck: Boolean): Boolean {
        ...
        return if (whiteTurn) {
            when (pieceKind) {
                ...
                PieceKind.WhiteBishop -> moveBishop(oldPos, newPos, pieceOnNewPos, withCheck)
            ...
            when (pieceKind) {
                ...
                PieceKind.BlackBishop -> moveBishop(oldPos, newPos, pieceOnNewPos, withCheck)
                else -> false
            }
        }
    }
    ...
    private fun moveBishop(
        ...
        if (rowDiff == colDiff) {
            if (pieceOnNewPos == null) {
                ...
                println("Cell checked: ${decodePosition(board[newPos.second][newPos.first].pos)}")
                return true
            } else if (pieceOnNewPos.color != color) {
                if (withCheck) {
                    removePiece(pieceOnNewPos)
            ...
            if (pieceOnNewPos !=null && pieceOnNewPos.color == color) {
                println("Piece on new position pos: ${decodePosition(pieceOnNewPos.position)}")
